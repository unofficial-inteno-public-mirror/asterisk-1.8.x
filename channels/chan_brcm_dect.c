			...
			if (!rDetected) {
				...
				//if (brcm_should_relay_dtmf(sub)) {
					...
					case EPDTMFRFC2833_DISABLED:
						ast_log(LOG_WARNING, "DTMF INBAND FOR DECT\n");
						brcm_signal_dtmf_ingress(sub, dtmfMap->i);
						break;
					case EPDTMFRFC2833_ENABLED:
						ast_log(LOG_WARNING, "DTMF RFC2833 FOR DECT\n");
						if (owner) {
							ast_queue_frame(owner, &f);
						}
						break;
					case EPDTMFRFC2833_SUBTRACT:
						ast_log(LOG_WARNING, "DTMF RFC2833 SUBTRACT FOR DECT\n");
						if (owner) {
							ast_queue_frame(owner, &f);
						}
						break;
					default:
						ast_log(LOG_WARNING, "DTMF UNKNOWN FOR DECT\n");
						break;
					}

					/*
					 * What is actually generated by this? Is it always inband tones,
					 * or is it dependant on how the endpoint was confed?
					 * In that case, maybe we can use only this and then let chan_brcm
					 * handle it same as a fxs?
					 *
					 * Manual states "generates DTMF tone"...
					 *
					 *
					 * Otherwise:
					 * RFC2833 och SIPInfo funkar med det som fanns.
					 * Vi borde bara behöva generera ton vid inband,
					 * vilket är = RCF2833_DISABLED.
					 *
					 */
				}
			}
